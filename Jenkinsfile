...
pipeline {
    ...
    stages {
        ...
        stage("deploy") {
            ...
            steps {
                script {
                    sh """
                        mvn -Dbuild_number=${BUILD_NUMBER} -Dmaven.repo.local=\$WORKSPACE/.repo -P native -pl cli build-helper:attach-artifact deploy:deploy
                    """
                }
            }
        }
    ...
    post {
        ...
        success {
            archiveArtifacts artifacts: 'cli/target/mconv,cli/target/npm-dist/*.tgz', fingerprint: true
        }
    }
}
