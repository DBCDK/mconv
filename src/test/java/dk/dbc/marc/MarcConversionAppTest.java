...
class MarcConversionAppTest {
    ...
    void marcXmlLine() throws Exception {
        String capturedStdout = tapSystemOut(() -> {
            MarcConversionApp.runWith(new String[]{"src/test/resources/marcxml_minimal.xml", "--format=LINE"});
        });
    ...
    void marcMmlLineWhiteSpaceShort() throws Exception {
        String capturedStdout = tapSystemOut(() -> {
            MarcConversionApp.runWith(new String[]{"src/test/resources/marcxml_minimal.xml", "-p","--format=LINE"});
        });

        assertThat(capturedStdout, is(new String("001 control1\n100    $a code-a $b code-b\n\n")));
    }
    ...
    void marcXmlLineWhiteSpaceLong() throws Exception {
        String capturedStdout = tapSystemOut(() -> {
            MarcConversionApp.runWith(new String[]{"src/test/resources/marcxml_minimal.xml", "--include-whitespace-padding","--format=LINE"});
        });
    ...
    void marcXmlLineWithLeaderShort() throws Exception {
        String capturedStdout = tapSystemOut(() -> {
            MarcConversionApp.runWith(new String[]{"src/test/resources/marcxml_minimal.xml", "-l","--format=LINE"});
        });
    ...
    void marcXmlLineWithLeaderLong() throws Exception {
        String capturedStdout = tapSystemOut(() -> {
            MarcConversionApp.runWith(new String[]{"src/test/resources/marcxml_minimal.xml", "--include-leader","--format=LINE"});
        });
    ...
    void marc8isoOutput() throws Exception {
        String capturedStdout = tapSystemOut(() -> {
            MarcConversionApp.runWith(new String[]{"src/test/resources/record_with_utf8.xml", "-o", "marc8", "--format=iso"});
        });

        assertThat(capturedStdout, is(readResourceAsString("src/test/resources/record_with_utf8_in_marc8.iso", StandardCharsets.UTF_8)));
    }

    @Test void marc8Input() throws Exception {
        String capturedStdout = tapSystemOut(() -> {
            MarcConversionApp.runWith(new String[]{"src/test/resources/record_with_utf8_in_marc8.iso", "-i", "marc8", "--format=line"});
        });
    ...
    @Test void danmarc2isoOutput() throws Exception {
        String capturedStdout = tapSystemOut(() -> {
            MarcConversionApp.runWith(new String[]{"src/test/resources/record_with_utf8.xml", "-o", "danmarc2", "--format=iso"});
        });

        assertThat(capturedStdout, is(readResourceAsString("src/test/resources/record_with_utf8_in_danmarc2.iso", StandardCharsets.UTF_8)));
    }

    @Test
    void danmarc2Input() throws Exception {
        String capturedStdout = tapSystemOut(() -> {
            MarcConversionApp.runWith(new String[]{"src/test/resources/record_with_utf8_in_danmarc2.iso", "-i", "danmarc2", "--format=line"});
        });
